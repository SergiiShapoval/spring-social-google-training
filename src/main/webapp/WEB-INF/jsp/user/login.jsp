...
<sec:authorize access="isAnonymous()">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <div class="row social-button-row">
                <div class="col-lg-4">
                    <a href="${pageContext.request.contextPath}/auth/google?scope=email%20profile&access_type=offline"><button class="btn btn-google-plus"><i class="icon-google-plus"></i> | <spring:message code="label.google.sign.in.button"/></button></a>
                </div>
            </div>
            <form action="<c:url value="/auth/google" />" method="POST">
                <input type="hidden" name="scope" value="https://www.googleapis.com/auth/plus.login https://www.googleapis.com/auth/plus.profile.emails.read https://www.googleapis.com/auth/userinfo.email" />
                <%--<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>--%>
                <button type="submit" class="btn btn-lg btn-google"><i class="icon-google-plus"></i> | Sign Up with Google</button>
            </form>
